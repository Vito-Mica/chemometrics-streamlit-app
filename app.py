...
if not st.session_state.authenticated:
    ...
    with _login_col:
        ...
        st.markdown(
            "<h2 style='text-align:center; color:#2c3e6b;'>🪨 RFA / XRF Mica Explorer</h2>"
            "<p style='text-align:center; color:#6c7a89;'>Bitte Passwort eingeben, um fortzufahren.</p>",
            unsafe_allow_html=True,
        )
        with st.form("login_form"):
            _pw = st.text_input("Passwort", type="password", placeholder="Passwort eingeben …")
            _submit = st.form_submit_button("🔓 Anmelden", use_container_width=True)
        if _submit:
            try:
                ...
                st.error("⚠️ `APP_PASSWORD` ist nicht in den Streamlit Secrets konfiguriert. "
                         "Bitte `.streamlit/secrets.toml` anlegen.")
    st.stop()
...
_ds_options = ["📤 Eigene Datei hochladen", "📦 Demo-Datensatz verwenden"]
_ds_idx = 0 if st.session_state.data_source == "upload" else 1
_ds_choice = st.sidebar.radio("Datenquelle", _ds_options, index=_ds_idx,
                               label_visibility="collapsed")
_new_source = "upload" if "Eigene" in _ds_choice else "demo"
...
if st.session_state.data_source == "demo":
    st.sidebar.caption("🟡 Demo-Modus aktiv")
else:
    st.sidebar.caption("Eigene Excel-/CSV-Datei in Schritt 1.1 hochladen")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if _page == EXP_PAGES[0]:
        ...
        def _process_and_join(df_d, df_k, source_label="Datei"):
            ...
            st.success(f"✅ {dm.shape[0]} Proben · {len(fc)} Features  ({source_label})")
            if len(match_report):
                ...
                if unm:
                    st.warning(f"⚠️ Kein Match: {unm}")
                else:
                    st.success("✅ Alle Herkunftsorte erfolgreich gematcht!")

        # ── DEMO MODE ────────────────────────────────────────────────────
        if st.session_state.data_source == "demo":
            st.info("📦 **Demo-Modus** — Der integrierte Demo-Datensatz wird verwendet.  \n"
                    "Wechseln Sie in der Sidebar auf *Eigene Datei hochladen*, um eigene Daten zu nutzen.")
            if not DEMO_FILE.exists():
                st.error(f"⚠️ Demo-Datei nicht gefunden: `{DEMO_FILE}`\n\n"
                         "Bitte legen Sie eine Excel-Datei unter `data/demo.xlsx` im Repository ab "
                         "(mit Sheets *Daten* und *Klassen*).")
                st.stop()
            ...
            if not st.session_state.demo_loaded or st.session_state.df_merged is None:
                try:
                    ...
                    _process_and_join(df_d, df_k, source_label="Demo-Datensatz")
                    st.session_state.demo_loaded = True
                except Exception as e:
                    st.error(f"Fehler beim Laden der Demo-Datei: {e}")
            else:
                dm2 = st.session_state.df_merged
                st.success(f"✅ {dm2.shape[0]} Proben · {len(st.session_state.feature_cols)} Features  (Demo-Datensatz)")
            ...
            if uploaded:
                try:
                    ...
                    if ext in ("xlsx", "xls"):
                        ...
                        k_opt = [s for s in xls.sheet_names if "klass" in s.lower()]
                        with c1:
                            sh_d = st.selectbox("Daten-Sheet", xls.sheet_names,
                                                index=xls.sheet_names.index(d_opt[0]) if d_opt else 0)
                        with c2:
                            sh_k = st.selectbox("Klassen-Sheet", xls.sheet_names,
