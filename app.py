            ...
            if st.button("🔬 Compute PLS-DA", type="primary"):
                with st.spinner("PLS-DA …"):
                    ...
                    if _n_lv >= 2:
                        ...
                        st.info("ℹ️ **Only one latent variable extracted.** "
                                "The separation is one-dimensional — LV1 is shown as a scatter plot.")
                        sc_all = pd.concat([
                            ...
                            ignore_index=True)
                        fig_pls = px.scatter(sc_all, x=cl, y="LV1", color=HERKUNFT_COL,
                                             symbol="Set", color_discrete_map=pal_color_map(p),
